...
const server = setupServer(
  ...
  http.get(`${REGISTRY_URL}/styles/hui/button.json`, () => {
    return HttpResponse.json({
      name: "button",
      type: "registry:ui",
      dependencies: ["@radix-ui/react-slot"],
...
describe("getRegistryItem", () => {
  it("should read and parse a valid local JSON file", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(tmpdir(), "hui-test-"))
  ...
  it("should handle tilde (~) home directory paths", async () => {
    ...
    const tempFile = path.join(homeDir, "hui-test-tilde.json")
    ...
    try {
      ...
      const tildeePath = "~/hui-test-tilde.json"
  ...
  it("should include error code in RegistryNotFoundError", async () => {
    server.use(
      http.get(`${REGISTRY_URL}/styles/hui/non-existent.json`, () => {
  ...
  it("should include error code in RegistryUnauthorizedError", async () => {
    server.use(
      http.get(`${REGISTRY_URL}/styles/hui/protected.json`, () => {
  ...
  it("should include error code in RegistryForbiddenError", async () => {
    server.use(
      http.get(`${REGISTRY_URL}/styles/hui/forbidden.json`, () => {
  ...
  it("should include error code in RegistryFetchError for 500 errors", async () => {
    server.use(
      http.get(`${REGISTRY_URL}/styles/hui/server-error.json`, () => {
        return HttpResponse.json(
  ...
  it("should serialize error to JSON correctly", async () => {
    server.use(
      http.get(`${REGISTRY_URL}/styles/hui/test-json.json`, () => {
        return HttpResponse.json({ error: "Not found" }, { status: 404 })
      }),
    )
  ...
  it("should include timestamp in errors", async () => {
    server.use(
      http.get(`${REGISTRY_URL}/styles/hui/timestamp-test.json`, () => {
        return HttpResponse.json({ error: "Not found" }, { status: 404 })
      }),
    )
  ...
  it("should include response body in RegistryFetchError", async () => {
    ...
    server.use(
      http.get(`${REGISTRY_URL}/styles/hui/bad-request.json`, () => {
...
describe("getRegistry", () => {
  ...
  it("should distinguish between URL and registry name", async () => {
    ...
    const registryName = "@hui"
    const registryUrl = "http://localhost:3000/registry.json"
    ...
    expect(urlResult.name).toBe("hui-from-url")
...
describe("getRegistriesConfig", () => {
  ...
  it("should fail when overriding built-in registries", async () => {
    ...
    const config = {
      ...
      registries: {
        "@hui": "http://localhost:3000/r/styles/{style}/{name}.json",
      },
    }
    ...
    try {
      ...
      if (error instanceof ConfigParseError) {
        ...
        expect(error.message).toContain(
          "\"@hui\" is a built-in registry and cannot be overridden",
        )
      }
    }
  })
