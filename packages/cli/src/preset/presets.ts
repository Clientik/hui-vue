...
// Named presets available via `init --preset <name>`. HUI ships a single
// style, so there is a single preset to go with it.
...
export async function promptForPreset(options: {
  ...
  if (selectedPreset === 'custom') {
    ...
    await promptToOpenPresetBuilder({
      createUrl,
      followUp: `Then ${highlighter.info('copy and run the command')} from localhost:3000.`,
    })
    return { kind: 'custom' }
  }
...
function isHuiVueInitUrl(url: string) {
  if (!isUrl(url))
    return false
