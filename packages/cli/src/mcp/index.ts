        ...
        inputSchema: zodToJsonSchema(
          z.object({
            registries: z
              .array(z.string())
              .describe(
                'Array of registry names to search (e.g., [\'@hui\', \'@acme\'])',
              ),
            limit: z
              .number()
              .optional()
        ...
        inputSchema: zodToJsonSchema(
          z.object({
            items: z
              .array(z.string())
              .describe(
                'Array of item names with registry prefix (e.g., [\'@hui/button\', \'@hui/card\'])',
              ),
          }),
        ),
      },
        ...
        inputSchema: zodToJsonSchema(
          z.object({
            items: z
              .array(z.string())
              .describe(
                'Array of items to get the add command for prefixed with the registry name (e.g., [\'@hui/button\', \'@hui/card\'])',
              ),
          }),
        ),
      },
          ...
          content: [
            {
              ...
              text: dedent`The following registries are configured in the current project:
                ...
                \`${await npxHuiVue('view @name-of-registry')}\`

                For example: \`${await npxHuiVue(
                  'view @hui',
                )}\` or \`${await npxHuiVue(
                  'view @hui @acme',
            ...
            content: [
              {
                ...
                Make sure the item names are correct and include the registry prefix (e.g., @hui/button).`,
              },
            ],
          }
        }
      ...
      case 'get_add_command_for_items': {
        ...
        return {
          content: [
            {
              type: 'text',
              text: await npxHuiVue(`add ${args.items.join(' ')}`),
            },
          ],
        }
      }
