...
const Component = props.type === 'block'
  ? defineAsyncComponent({
      loader: () => import(`@/registry/hui/blocks/${props.name}/page.vue`),
    })
  : props.name.toLowerCase().includes('chart') && !props.name.toLocaleLowerCase().includes('demo')
    ? defineAsyncComponent({
        loader: () => import(`@/registry/hui/charts/${props.name}.vue`),
      })
    : defineAsyncComponent({
...
<template>
  ...
  <div v-else-if="type === 'block'" class="relative aspect-[4/2.5] w-full overflow-hidden rounded-md border md:-mx-1">
    <img
      :src="`/r/styles/hui/${name}-light.png`"
      :alt="name"
      width="1440"
      height="900"
    ...
    >
    <img
      :src="`/r/styles/hui/${name}-dark.png`"
      :alt="name"
      :width="1440"
      :height="900"
