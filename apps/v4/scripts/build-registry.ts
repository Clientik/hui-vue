...
import { runEslintFix, runHuiVueCli, runPrettierWrite } from './lib/run-formatters'
...
export const Index: Record<string, any> = {`
  ...
  for (const item of registry.items) {
    ...
    const componentPath = item.files?.[0]?.path
      ? `@/registry/hui/${item.files[0].path}`
      : ''
  ...
  "${item.name}": {
    ...
    files: [${item.files?.map((file) => {
      const filePath = `registry/hui/${typeof file === 'string' ? file : file.path}`
...
async function buildRegistry(base: RegistryBase, tempRegistryPath: string) {
  ...
  try {
    await runHuiVueCli(['build', tempRegistryPath, '--output', base.publicOutputDir!])
    return { success: true }
  }
  catch (err: any) {
