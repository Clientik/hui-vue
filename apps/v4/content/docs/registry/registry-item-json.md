...
You can see the JSON Schema for `registry-item.json` [here](https://clientik.github.io/hui-vue/schema/registry-item.json).
...
- For `hui-kit` registry items such as `button`, `input`, `select`, etc use the name eg. `['button', 'input', 'select']`.
...
By default, the `hui-kit` cli will read a project's `components.json` file to determine the target path. For some files, such as routes or config you can specify the target path manually.
