...
You can see the JSON Schema for `registry-item.json` [here](http://localhost:3000/schema/registry-item.json).
...
- For `hui-vue` registry items such as `button`, `input`, `select`, etc use the name eg. `['button', 'input', 'select']`.
...
By default, the `hui-vue` cli will read a project's `components.json` file to determine the target path. For some files, such as routes or config you can specify the target path manually.
