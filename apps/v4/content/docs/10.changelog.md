...
We ported **hui/typeset** to fix that. It's one CSS file that styles everything inside a `typeset` container, and it lives in your project, so you can change it whenever you need to.
...
Say hello to **Rhea**, a new style for hui-kit.

Rhea is the latest hui-kit look, mirroring the upstream `base-rhea` style as closely as possible. It keeps the rounded, soft aesthetic you already know but refines the details: denser surfaces, ring-based focus states, and subtle press interactions on buttons. Built for focused product interfaces.
...
Two CSS utilities to finish off the set. [`scroll-fade`](/docs/utilities/scroll-fade) dissolves content at the edges of a scroll container, and [`shimmer`](/docs/utilities/shimmer) adds a shimmer effect to text, perfect for streaming and loading states. Both ship with the `hui-kit` package, so there's nothing to install after `init`.
...
We've upgraded [localhost:3000](/) to Nuxt v4 and Tailwind v4. The site now uses the upgraded `new-york` components.
...
## February 2025 - Reka UI & npx hui-kit@latest init

We've updated the latest registry to support Reka UI instead of Radix Vue.

The updated CLI is now available. You can now install components, themes, composables, utils and more using `npx hui-kit add`.

This is a major step towards distributing code that you and your LLMs can access and use.

::callout
With the released of [Reka UI v2](https://reka-ui.com/), `hui-kit@latest` command will now install Reka UI. If you want to keep using [Radix Vue](https://radix-vue.com/), please visit [here](https://radix.localhost:3000/) and run `hui-kit@radix` command instead.
::
...
3. You can also install remote components using url. `npx hui-kit add https://acme.com/registry/navbar.json`.
<!-- 4. We have also improve the init command. It does framework detection and can even init a brand new Next.js app in one command. `npx hui init`. -->
...
```bash
npx hui-kit@latest init Sidebar01 Login01
```
