...
Say hello to **Rhea**, a new style for hui-vue.

Rhea is the latest hui-vue look, mirroring the upstream `base-rhea` style as closely as possible. It keeps the rounded, soft aesthetic you already know but refines the details: denser surfaces, ring-based focus states, and subtle press interactions on buttons. Built for focused product interfaces.
...
Two CSS utilities to finish off the set. [`scroll-fade`](/docs/utilities/scroll-fade) dissolves content at the edges of a scroll container, and [`shimmer`](/docs/utilities/shimmer) adds a shimmer effect to text, perfect for streaming and loading states. Both ship with the `hui-vue` package, so there's nothing to install after `init`.
...
## February 2025 - Reka UI & npx hui-vue@latest init

We've updated the latest registry to support Reka UI instead of Radix Vue.

The updated CLI is now available. You can now install components, themes, composables, utils and more using `npx hui-vue add`.

This is a major step towards distributing code that you and your LLMs can access and use.

::callout
With the released of [Reka UI v2](https://reka-ui.com/), `hui-vue@latest` command will now install Reka UI. If you want to keep using [Radix Vue](https://radix-vue.com/), please visit [here](https://radix.localhost:3000/) and run `hui-vue@radix` command instead.
::
...
3. You can also install remote components using url. `npx hui-vue add https://acme.com/registry/navbar.json`.
...
```bash
npx hui-vue@latest init Sidebar01 Login01
```
