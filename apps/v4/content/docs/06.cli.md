---
title: CLI
description: Use the hui-kit CLI to add components to your project.
---
...
```bash
Usage: hui-kit init [options] [components...]
...
```bash
npx hui-kit@latest add [component]
```
...
| Form | Resolves to |
| --- | --- |
| `button` | an item in the default `hui-kit` registry |
...
```bash
Usage: hui-kit add [options] [components...]
...
```bash
Usage: hui-kit apply [options] [preset]
...
```bash
npx hui-kit@latest view [item]
```

You can view multiple items at once:

```bash
npx hui-kit@latest view button card dialog
```

Or view items from namespaced registries:

```bash
npx hui-kit@latest view @acme/auth @v0/dashboard
```

**Options**

```bash
Usage: hui-kit view [options] <items...>
...
```bash
npx hui-kit@latest search [registry]
```

You can search with a query:

```bash
npx hui-kit@latest search @hui-kit -q "button"
```

Or search multiple registries at once:

```bash
npx hui-kit@latest search @hui-kit @v0 @acme
```

The `list` command is an alias for `search`:

```bash
npx hui-kit@latest list @acme
```

**Options**

```bash
Usage: hui-kit search|list [options] <registries...>
...
```bash
Usage: hui-kit build [options] [registry]

build components for a hui-kit registry
...
```bash
npx hui-kit@latest build --output ./public/registry
```
...
```bash
npx hui-kit@latest docs [component]
```

**Options**

```bash
Usage: hui-kit docs [options] <components...>
...
```bash
npx hui-kit@latest info
```

**Options**

```bash
Usage: hui-kit info [options]
...
```bash
npx hui-kit@latest migrate [migration]
```
...
```bash
Usage: hui-kit migrate [options] [migration] [path]
...
```bash
npx hui-kit@latest migrate rtl
```
...
npx hui-kit@latest migrate rtl src/components/ui/button/Button.vue

# Migrate files matching a glob pattern
npx hui-kit@latest migrate rtl "src/components/ui/**"
```
