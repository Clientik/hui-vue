...
::vue-school-link{lesson="theming-with-hui-kit" placement="top"}
Watch a Vue School video about theming with hui-kit.
::

<Callout>

Want to build your theme visually? Use [hui-kit/create](/create) to preview colors, radius, fonts, and icons, then generate a preset for your project.
...
```bash
npx hui-kit@latest init --no-css-variables
```
