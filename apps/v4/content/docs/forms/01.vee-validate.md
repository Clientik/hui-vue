...
::vue-school-link{class="mt-6" lesson="forms-and-form-validation-with-hui-kit" placement="top"}
Watch a Vue School video about building forms and validation with hui-kit.
::
...
- hui-kit `<Field />` components for building accessible forms.
- Client-side validation using Zod with `toTypedSchema`.

## Anatomy

Here's a basic example of a form using VeeValidate's `<Field />` component with scoped slots and hui-kit `<Field />` components.

Bind `componentField` when the control is a Vue component with `v-model`, such as hui-kit's `<Input />`. Bind `field` when the control is a native element. `field` binds `value`, which a `v-model` component ignores, so `initialValues` never render.
...
We can now build the form using VeeValidate's `<Field />` component with scoped slots and hui-kit `<Field />` components.
...
- Add the `:data-invalid` prop to the hui-kit `<Field />` component.
...
- To show errors, add the `:aria-invalid` prop to the `<Input />` component and the `:data-invalid` prop to the hui-kit `<Field />` component.
...
- To show errors, add the `:aria-invalid` prop to the `<Textarea />` component and the `:data-invalid` prop to the hui-kit `<Field />` component.
...
- To show errors, add the `:aria-invalid` prop to the `<SelectTrigger />` component and the `:data-invalid` prop to the hui-kit `<Field />` component.
...
- To show errors, add the `:aria-invalid` prop to the `<Checkbox />` component and the `:data-invalid` prop to the hui-kit `<Field />` component.
...
- To show errors, add the `:aria-invalid` prop to the `<RadioGroup />` component and the `:data-invalid` prop to the hui-kit `<Field />` component.
...
- To show errors, add the `:aria-invalid` prop to the `<Switch />` component and the `:data-invalid` prop to the hui-kit `<Field />` component.
