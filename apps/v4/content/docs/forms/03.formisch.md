...
Here's a basic example of a form using the `Field` component from Formisch and the hui-kit `Field` component.
...
icon: true
---
**Note:** Formisch ships its own `Field` component. To avoid a name clash with the hui-kit `Field`, the examples below import the Formisch one as `FormischField` and keep the hui-kit `Field` under its original name. In your own code you can alias either side — just be consistent.
::
