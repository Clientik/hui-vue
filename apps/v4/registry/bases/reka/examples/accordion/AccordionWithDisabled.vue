...
<template>
  <Example title="With Disabled">
    <Accordion
      type="single"
      collapsible
      class="style-lyra:rounded-none style-vega:rounded-lg style-neva:rounded-lg style-maia:rounded-lg style-mira:rounded-lg mx-auto max-w-lg overflow-hidden border"
    >
      <AccordionItem
        v-for="item in items"
        :key="item.value"
      ...
      >
        <AccordionTrigger class="style-neva:px-2.5 style-lyra:px-2 style-vega:px-4">
          {{ item.trigger }}
        </AccordionTrigger>
        <AccordionContent class="style-neva:px-2.5 style-lyra:px-2 style-vega:px-4">
          {{ item.content }}
        </AccordionContent>
      </AccordionItem>
    </Accordion>
